...
export class AdminBlogForm implements OnInit {
  ...
  categories = ['Implants', 'Cosmetic', 'Orthodontics', 'Pediatric', 'Lifestyle', 'Hygiene', 'General'];
    ...
    if (id) {
      ...
      try {
        ...
        this.form.patchValue({
          ...
          readTime: p.readTime, color: p.color,
          image: p.image ?? '', content: p.content
        });
  ...
  async submit() {
    ...
    try {
      ...
      const payload = {
        ...
        category: v.category, author: v.author, date: v.date,
        readTime: v.readTime, color: v.color,
        image: v.image || undefined, content: v.content
      };
      if (this.editingId()) {
